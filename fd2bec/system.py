...
class System:
    ...
    def __post_init__(self):
        ...
        A = self.unit_cell.to_fractional(self.displacements.reshape((nr, len(self.unit_cell), 3)), rank=1).reshape((nr, -1))
        ...
        # if self.use_spacegroup_symmetry:
            ...
            # x = np.asarray([f"d mu_{i} / d R_{j}^{k}" for i in range(3) for j in range(3) for k in range(len(self.unit_cell))], dtype=object)
