...
def prepare_args(descr):
    ...
    parser.add_argument(
        ...
        **argv,
        type=int,
        required=True,
        help="number of random displacements to generate",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def main(args):
    ...
    print(
        f"Generating {N} random numbers for {args.number} displaced structures ... ",
        end="",
    )
    displacements = np.random.normal(
        scale=args.amplitude, size=(args.number, atoms.get_global_number_of_atoms() * 3)
    )
    print("done")
